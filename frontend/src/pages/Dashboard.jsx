  ...
  return (
    ...
    >
      ...
      >
        ...
        {nestedDisabled && (
          <p className="mb-4 rounded-lg bg-surface-container-high px-3 py-2 text-xs text-on-surface-variant">
...
export default function DashboardPage() {
  ...
  const visibleFiles = useMemo(() => {
    return files.filter(
      (file) => file.name !== ".__folder__" && (file.directory || "") === currentFolder
    );
  }, [files, currentFolder]);
  ...
  const filteredFolders = useMemo(() => {
    ...
    return visibleFolders.filter((folderPath) =>
      folderPath.split("/").pop()?.toLowerCase().includes(query)
    );
        ...
        <main className="flex-1 overflow-y-auto bg-gradient-to-br from-background to-surface-container-lowest px-4 pb-8 pt-24 md:px-8">
          <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              ...
              {currentFolder && (
                <p className="mt-1 text-sm text-on-surface-variant">{currentFolder}</p>
              )}
            </div>
          ...
          <div className="glass-panel relative mb-8 flex flex-col items-center justify-between gap-4 overflow-hidden rounded-2xl border border-outline-variant/10 p-4 sm:flex-row">
                  ...
                  <div>
                    ...
                    <div className="glass-panel overflow-hidden rounded-2xl border border-outline-variant/10 bg-surface-container-lowest">
                      <div className="grid grid-cols-12 gap-2 border-b border-outline-variant/10 bg-surface-container-low/50 p-3 px-4">
                        <div className="col-span-6 font-geist text-xs uppercase tracking-widest text-on-surface-variant sm:col-span-5">
                          Name
                        </div>
                        <div className="col-span-3 hidden font-geist text-xs uppercase tracking-widest text-on-surface-variant sm:block">
                          Location
                        </div>
                        <div className="col-span-3 font-geist text-xs uppercase tracking-widest text-on-surface-variant sm:col-span-2">
                          Data Center
                        </div>
